...
-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
--
-- Generated on: 2023-08-10 02:18:30
...
BEGIN
	...
	sql := format('ALTER EXTENSION postgis_raster ADD %s', obj);
	EXECUTE sql;
	RAISE NOTICE 'newly registered %', obj;

EXCEPTION
WHEN object_not_in_prerequisite_state THEN
  IF SQLERRM ~ '\mpostgis_raster\M'
  THEN
...
WHEN OTHERS THEN
	RAISE EXCEPTION 'Trying to add % to postgis_raster, got % (%)', obj, SQLERRM, SQLSTATE;
END;
$$ LANGUAGE 'plpgsql';
-- Register all views.
SELECT _postgis_package_object('VIEW', 'raster_columns');
SELECT _postgis_package_object('VIEW', 'raster_overviews');
-- Register all tables.
-- Register all sequences.
-- Register all aggregates.
SELECT _postgis_package_object('AGGREGATE', 'st_summarystatsagg (raster, integer, boolean, double precision)');
SELECT _postgis_package_object('AGGREGATE', 'st_summarystatsagg (raster, boolean, double precision)');
SELECT _postgis_package_object('AGGREGATE', 'st_summarystatsagg (raster, int, boolean)');
SELECT _postgis_package_object('AGGREGATE', 'st_countagg (raster, integer, boolean, double precision)');
SELECT _postgis_package_object('AGGREGATE', 'st_countagg (raster, integer, boolean)');
SELECT _postgis_package_object('AGGREGATE', 'st_countagg (raster, boolean)');
SELECT _postgis_package_object('AGGREGATE', 'st_samealignment (raster)');
SELECT _postgis_package_object('AGGREGATE', 'st_union (raster, unionarg[])');
SELECT _postgis_package_object('AGGREGATE', 'st_union (raster, integer, text)');
SELECT _postgis_package_object('AGGREGATE', 'st_union (raster, integer)');
SELECT _postgis_package_object('AGGREGATE', 'st_union (raster)');
SELECT _postgis_package_object('AGGREGATE', 'st_union (raster, text)');
-- Register all operators classes and families.
SELECT _postgis_package_object('OPERATOR', 'CLASS hash_raster_ops USING hash');
SELECT _postgis_package_object('OPERATOR', 'FAMILY hash_raster_ops USING hash');
-- Register all operators.
SELECT _postgis_package_object('OPERATOR', '=  (raster,raster)');
SELECT _postgis_package_object('OPERATOR', '<<  (raster,raster)');
SELECT _postgis_package_object('OPERATOR', '&<  (raster,raster)');
SELECT _postgis_package_object('OPERATOR', '<<|  (raster,raster)');
SELECT _postgis_package_object('OPERATOR', '&<|  (raster,raster)');
...
SELECT _postgis_package_object('OPERATOR', '@  (raster,raster)');
SELECT _postgis_package_object('OPERATOR', '~  (raster,raster)');
SELECT _postgis_package_object('OPERATOR', '~  (raster,geometry)');
SELECT _postgis_package_object('OPERATOR', '@  (raster,geometry)');
SELECT _postgis_package_object('OPERATOR', '&&  (raster,geometry)');
SELECT _postgis_package_object('OPERATOR', '~  (geometry,raster)');
SELECT _postgis_package_object('OPERATOR', '@  (geometry,raster)');
SELECT _postgis_package_object('OPERATOR', '&&  (geometry,raster)');
-- Register all casts.
SELECT _postgis_package_object('CAST', '(raster AS box3d)');
SELECT _postgis_package_object('CAST', '(raster AS geometry)');
SELECT _postgis_package_object('CAST', '(raster AS bytea)');
-- Register all functions.
SELECT _postgis_package_object('FUNCTION', 'raster_in (cstring)');
SELECT _postgis_package_object('FUNCTION', 'raster_out (raster)');
SELECT _postgis_package_object('FUNCTION', 'postgis_raster_lib_version ()');
SELECT _postgis_package_object('FUNCTION', 'postgis_raster_scripts_installed ()');
SELECT _postgis_package_object('FUNCTION', 'postgis_raster_lib_build_date ()');
SELECT _postgis_package_object('FUNCTION', 'postgis_gdal_version ()');
SELECT _postgis_package_object('FUNCTION', 'st_envelope (raster)');
SELECT _postgis_package_object('FUNCTION', 'st_convexhull (raster)');
SELECT _postgis_package_object('FUNCTION', 'st_minconvexhull (
	rast raster,
	nband integer )');
SELECT _postgis_package_object('FUNCTION', 'box3d (raster)');
SELECT _postgis_package_object('FUNCTION', 'st_height (raster)');
SELECT _postgis_package_object('FUNCTION', 'st_numbands (raster)');
SELECT _postgis_package_object('FUNCTION', 'st_scalex (raster)');
SELECT _postgis_package_object('FUNCTION', 'st_scaley (raster)');
SELECT _postgis_package_object('FUNCTION', 'st_skewx (raster)');
SELECT _postgis_package_object('FUNCTION', 'st_skewy (raster)');
SELECT _postgis_package_object('FUNCTION', 'st_srid (raster)');
SELECT _postgis_package_object('FUNCTION', 'st_upperleftx (raster)');
SELECT _postgis_package_object('FUNCTION', 'st_upperlefty (raster)');
SELECT _postgis_package_object('FUNCTION', 'st_width (raster)');
SELECT _postgis_package_object('FUNCTION', 'st_pixelwidth (raster)');
SELECT _postgis_package_object('FUNCTION', 'st_pixelheight (raster)');
SELECT _postgis_package_object('FUNCTION', 'st_geotransform (raster,
...
SELECT _postgis_package_object('FUNCTION', 'st_rotation (raster)');
SELECT _postgis_package_object('FUNCTION', 'st_metadata (
	rast raster,
...
SELECT _postgis_package_object('FUNCTION', 'st_summary (rast raster)');
SELECT _postgis_package_object('FUNCTION', 'ST_MemSize (raster)');
SELECT _postgis_package_object('FUNCTION', 'st_makeemptyraster (width int, height int, upperleftx float8, upperlefty float8, scalex float8, scaley float8, skewx float8, skewy float8, srid integer )');
SELECT _postgis_package_object('FUNCTION', 'st_makeemptyraster (width int, height int, upperleftx float8, upperlefty float8, pixelsize float8)');
SELECT _postgis_package_object('FUNCTION', 'st_makeemptyraster (rast raster)');
SELECT _postgis_package_object('FUNCTION', 'st_addband (rast raster, addbandargset addbandarg[])');
SELECT _postgis_package_object('FUNCTION', 'st_addband (
	rast raster,
	index int,
	pixeltype text,
	initialvalue float8 ,
...
SELECT _postgis_package_object('FUNCTION', 'st_addband (
	...
	index int ,
	nodataval double precision )');
SELECT _postgis_package_object('FUNCTION', 'st_band (rast raster, nbands int[] )');
SELECT _postgis_package_object('FUNCTION', 'st_band (rast raster, nband int)');
SELECT _postgis_package_object('FUNCTION', 'st_band (rast raster, nbands text, delimiter char )');
SELECT _postgis_package_object('FUNCTION', '_st_summarystats (
	rast raster,
	nband int ,
	exclude_nodata_value boolean ,
	sample_percent double precision )');
SELECT _postgis_package_object('FUNCTION', 'st_summarystats (
	rast raster,
	nband int ,
	exclude_nodata_value boolean )');
SELECT _postgis_package_object('FUNCTION', 'st_summarystats (
...
SELECT _postgis_package_object('FUNCTION', 'st_approxsummarystats (
	rast raster,
	nband int ,
	exclude_nodata_value boolean ,
	sample_percent double precision )');
...
SELECT _postgis_package_object('FUNCTION', '_st_summarystats_finalfn (internal)');
SELECT _postgis_package_object('FUNCTION', '_st_summarystats_transfn (
	internal,
	raster, integer,
...
SELECT _postgis_package_object('FUNCTION', '_st_count (rast raster, nband int , exclude_nodata_value boolean , sample_percent double precision )');
SELECT _postgis_package_object('FUNCTION', 'st_count (rast raster, nband int , exclude_nodata_value boolean )');
SELECT _postgis_package_object('FUNCTION', 'st_count (rast raster, exclude_nodata_value boolean)');
SELECT _postgis_package_object('FUNCTION', 'st_approxcount (rast raster, nband int , exclude_nodata_value boolean , sample_percent double precision )');
SELECT _postgis_package_object('FUNCTION', 'st_approxcount (rast raster, nband int, sample_percent double precision)');
SELECT _postgis_package_object('FUNCTION', 'st_approxcount (rast raster, exclude_nodata_value boolean, sample_percent double precision )');
SELECT _postgis_package_object('FUNCTION', 'st_approxcount (rast raster, sample_percent double precision)');
SELECT _postgis_package_object('FUNCTION', '_st_countagg_finalfn (agg agg_count)');
SELECT _postgis_package_object('FUNCTION', '__st_countagg_transfn (
	agg agg_count,
	rast raster,
 	nband integer , exclude_nodata_value boolean ,
	sample_percent double precision )');
SELECT _postgis_package_object('FUNCTION', '_st_countagg_transfn (
	agg agg_count,
	rast raster,
 	nband integer, exclude_nodata_value boolean,
	sample_percent double precision
...
SELECT _postgis_package_object('FUNCTION', '_st_histogram (
...
SELECT _postgis_package_object('FUNCTION', 'st_histogram (
	rast raster, nband int ,
	exclude_nodata_value boolean ,
	bins int , width double precision[] ,
	right boolean ,
...
SELECT _postgis_package_object('FUNCTION', 'st_approxhistogram (
	rast raster, nband int ,
	exclude_nodata_value boolean ,
	sample_percent double precision ,
	bins int , width double precision[] ,
...
SELECT _postgis_package_object('FUNCTION', '_st_quantile (
	rast raster,
	nband int ,
	exclude_nodata_value boolean ,
	sample_percent double precision ,
...
SELECT _postgis_package_object('FUNCTION', 'st_quantile (
	rast raster,
	nband int ,
	exclude_nodata_value boolean ,
	quantiles double precision[] ,
...
SELECT _postgis_package_object('FUNCTION', 'st_quantile (rast raster, nband int, exclude_nodata_value boolean, quantile double precision)');
SELECT _postgis_package_object('FUNCTION', 'st_quantile (rast raster, nband int, quantile double precision)');
SELECT _postgis_package_object('FUNCTION', 'st_quantile (rast raster, exclude_nodata_value boolean, quantile double precision )');
SELECT _postgis_package_object('FUNCTION', 'st_quantile (rast raster, quantile double precision)');
SELECT _postgis_package_object('FUNCTION', 'st_approxquantile (
	rast raster,
	nband int ,
	exclude_nodata_value boolean ,
	sample_percent double precision ,
...
SELECT _postgis_package_object('FUNCTION', 'st_approxquantile (rast raster, nband int, exclude_nodata_value boolean, sample_percent double precision, quantile double precision)');
SELECT _postgis_package_object('FUNCTION', 'st_approxquantile (rast raster, nband int, sample_percent double precision, quantile double precision)');
SELECT _postgis_package_object('FUNCTION', 'st_approxquantile (rast raster, sample_percent double precision, quantile double precision)');
SELECT _postgis_package_object('FUNCTION', 'st_approxquantile (rast raster, exclude_nodata_value boolean, quantile double precision )');
SELECT _postgis_package_object('FUNCTION', 'st_approxquantile (rast raster, quantile double precision)');
SELECT _postgis_package_object('FUNCTION', '_st_valuecount (
...
SELECT _postgis_package_object('FUNCTION', 'st_valuecount (
	rast raster, nband integer ,
	exclude_nodata_value boolean ,
	searchvalues double precision[] ,
	roundto double precision ,
...
SELECT _postgis_package_object('FUNCTION', 'st_valuecount (rast raster, nband integer, searchvalues double precision[], roundto double precision , OUT value double precision, OUT count integer)');
SELECT _postgis_package_object('FUNCTION', 'st_valuecount (rast raster, searchvalues double precision[], roundto double precision , OUT value double precision, OUT count integer)');
SELECT _postgis_package_object('FUNCTION', 'st_valuecount (rast raster, nband integer, exclude_nodata_value boolean, searchvalue double precision, roundto double precision )');
SELECT _postgis_package_object('FUNCTION', 'st_valuecount (rast raster, nband integer, searchvalue double precision, roundto double precision )');
SELECT _postgis_package_object('FUNCTION', 'st_valuecount (rast raster, searchvalue double precision, roundto double precision )');
SELECT _postgis_package_object('FUNCTION', 'st_valuepercent (
	rast raster, nband integer ,
	exclude_nodata_value boolean ,
	searchvalues double precision[] ,
	roundto double precision ,
...
SELECT _postgis_package_object('FUNCTION', 'st_valuepercent (rast raster, nband integer, searchvalues double precision[], roundto double precision , OUT value double precision, OUT percent double precision)');
SELECT _postgis_package_object('FUNCTION', 'st_valuepercent (rast raster, searchvalues double precision[], roundto double precision , OUT value double precision, OUT percent double precision)');
SELECT _postgis_package_object('FUNCTION', 'st_valuepercent (rast raster, nband integer, exclude_nodata_value boolean, searchvalue double precision, roundto double precision )');
SELECT _postgis_package_object('FUNCTION', 'st_valuepercent (rast raster, nband integer, searchvalue double precision, roundto double precision )');
SELECT _postgis_package_object('FUNCTION', 'st_valuepercent (rast raster, searchvalue double precision, roundto double precision )');
SELECT _postgis_package_object('FUNCTION', '_st_valuecount (
	rastertable text,
	rastercolumn text,
	nband integer ,
...
SELECT _postgis_package_object('FUNCTION', 'st_valuecount (rastertable text, rastercolumn text, nband integer, searchvalues double precision[], roundto double precision , OUT value double precision, OUT count integer)');
SELECT _postgis_package_object('FUNCTION', 'st_valuecount (rastertable text, rastercolumn text, searchvalues double precision[], roundto double precision , OUT value double precision, OUT count integer)');
SELECT _postgis_package_object('FUNCTION', 'st_valuecount (rastertable text, rastercolumn text, nband integer, exclude_nodata_value boolean, searchvalue double precision, roundto double precision )');
SELECT _postgis_package_object('FUNCTION', 'st_valuecount (rastertable text, rastercolumn text, nband integer, searchvalue double precision, roundto double precision )');
SELECT _postgis_package_object('FUNCTION', 'st_valuecount (rastertable text, rastercolumn text, searchvalue double precision, roundto double precision )');
SELECT _postgis_package_object('FUNCTION', 'st_valuepercent (
	rastertable text, rastercolumn text,
	nband integer ,
	exclude_nodata_value boolean ,
...
SELECT _postgis_package_object('FUNCTION', 'st_valuepercent (rastertable text, rastercolumn text, nband integer, searchvalues double precision[], roundto double precision , OUT value double precision, OUT percent double precision)');
SELECT _postgis_package_object('FUNCTION', 'st_valuepercent (rastertable text, rastercolumn text, searchvalues double precision[], roundto double precision , OUT value double precision, OUT percent double precision)');
SELECT _postgis_package_object('FUNCTION', 'st_valuepercent (rastertable text, rastercolumn text, nband integer, exclude_nodata_value boolean, searchvalue double precision, roundto double precision )');
SELECT _postgis_package_object('FUNCTION', 'st_valuepercent (rastertable text, rastercolumn text, nband integer, searchvalue double precision, roundto double precision )');
SELECT _postgis_package_object('FUNCTION', 'st_valuepercent (rastertable text, rastercolumn text, searchvalue double precision, roundto double precision )');
SELECT _postgis_package_object('FUNCTION', '_st_reclass (rast raster, VARIADIC reclassargset reclassarg[])');
SELECT _postgis_package_object('FUNCTION', 'st_reclass (rast raster, VARIADIC reclassargset reclassarg[])');
SELECT _postgis_package_object('FUNCTION', 'st_reclass (rast raster, nband int, reclassexpr text, pixeltype text, nodataval double precision )');
SELECT _postgis_package_object('FUNCTION', 'st_reclass (rast raster, reclassexpr text, pixeltype text)');
SELECT _postgis_package_object('FUNCTION', '_st_colormap (
	rast raster, nband int,
	colormap text,
	method text )');
SELECT _postgis_package_object('FUNCTION', 'st_colormap (
	rast raster, nband int ,
	colormap text ,
	method text )');
SELECT _postgis_package_object('FUNCTION', 'st_colormap (
	...
	colormap text,
	method text )');
SELECT _postgis_package_object('FUNCTION', 'st_fromgdalraster (gdaldata bytea, srid integer )');
SELECT _postgis_package_object('FUNCTION', 'st_gdaldrivers (
	OUT idx int,
	OUT short_name text,
	OUT long_name text,
...
SELECT _postgis_package_object('FUNCTION', 'st_asgdalraster (rast raster, format text, options text[] , srid integer )');
SELECT _postgis_package_object('FUNCTION', 'ST_Contour (
		rast raster,
		bandnumber integer ,
		level_interval float8 ,
		level_base float8 ,
		fixed_levels float8[] ,
		polygonize boolean )');
SELECT _postgis_package_object('FUNCTION', 'ST_InterpolateRaster (
		geom geometry,
		options text,
		rast raster,
		bandnumber integer )');
SELECT _postgis_package_object('FUNCTION', 'st_astiff (rast raster, options text[] , srid integer )');
SELECT _postgis_package_object('FUNCTION', 'st_astiff (rast raster, nbands int[], options text[] , srid integer )');
SELECT _postgis_package_object('FUNCTION', 'st_astiff (rast raster, compression text, srid integer )');
SELECT _postgis_package_object('FUNCTION', 'st_astiff (rast raster, nbands int[], compression text, srid integer )');
SELECT _postgis_package_object('FUNCTION', 'st_asjpeg (rast raster, options text[] )');
SELECT _postgis_package_object('FUNCTION', 'st_asjpeg (rast raster, nbands int[], options text[] )');
SELECT _postgis_package_object('FUNCTION', 'st_asjpeg (rast raster, nbands int[], quality int)');
SELECT _postgis_package_object('FUNCTION', 'st_asjpeg (rast raster, nband int, options text[] )');
SELECT _postgis_package_object('FUNCTION', 'st_asjpeg (rast raster, nband int, quality int)');
SELECT _postgis_package_object('FUNCTION', 'st_aspng (rast raster, options text[] )');
SELECT _postgis_package_object('FUNCTION', 'st_aspng (rast raster, nbands int[], options text[] )');
SELECT _postgis_package_object('FUNCTION', 'st_aspng (rast raster, nbands int[], compression int)');
SELECT _postgis_package_object('FUNCTION', 'st_aspng (rast raster, nband int, options text[] )');
SELECT _postgis_package_object('FUNCTION', 'st_aspng (rast raster, nband int, compression int)');
SELECT _postgis_package_object('FUNCTION', '_st_asraster (
	geom geometry,
	...
	touched boolean )');
SELECT _postgis_package_object('FUNCTION', 'st_asraster (
	geom geometry,
	scalex double precision, scaley double precision,
	gridx double precision , gridy double precision ,
	pixeltype text[] ,
...
SELECT _postgis_package_object('FUNCTION', 'st_asraster (
	...
	nodataval double precision ,
	touched boolean )');
SELECT _postgis_package_object('FUNCTION', '_ST_gdalwarp (
	rast raster,
	...
	skewx double precision , skewy double precision ,
	width integer , height integer )');
SELECT _postgis_package_object('FUNCTION', 'st_resample (
	rast raster,
	scalex double precision , scaley double precision ,
	gridx double precision , gridy double precision ,
	skewx double precision , skewy double precision ,
...
SELECT _postgis_package_object('FUNCTION', 'st_resample (
	...
	algorithm text ,
	maxerr double precision )');
SELECT _postgis_package_object('FUNCTION', 'st_transform (rast raster, srid integer, algorithm text , maxerr double precision , scalex double precision , scaley double precision )');
SELECT _postgis_package_object('FUNCTION', 'st_transform (rast raster, srid integer, scalex double precision, scaley double precision, algorithm text , maxerr double precision )');
SELECT _postgis_package_object('FUNCTION', 'st_transform (rast raster, srid integer, scalexy double precision, algorithm text , maxerr double precision )');
SELECT _postgis_package_object('FUNCTION', 'st_transform (
	rast raster,
	alignto raster,
	algorithm text , maxerr double precision )');
SELECT _postgis_package_object('FUNCTION', 'st_rescale (rast raster, scalex double precision, scaley double precision, algorithm text , maxerr double precision )');
SELECT _postgis_package_object('FUNCTION', 'st_rescale (rast raster, scalexy double precision, algorithm text , maxerr double precision )');
SELECT _postgis_package_object('FUNCTION', 'st_reskew (rast raster, skewx double precision, skewy double precision, algorithm text , maxerr double precision )');
SELECT _postgis_package_object('FUNCTION', 'st_reskew (rast raster, skewxy double precision, algorithm text , maxerr double precision )');
SELECT _postgis_package_object('FUNCTION', 'st_snaptogrid (
	rast raster,
	gridx double precision, gridy double precision,
	algorithm text , maxerr double precision ,
	scalex double precision , scaley double precision )');
...
SELECT _postgis_package_object('FUNCTION', 'st_snaptogrid (
	...
	algorithm text , maxerr double precision )');
SELECT _postgis_package_object('FUNCTION', 'st_resize (
	rast raster,
...
SELECT _postgis_package_object('FUNCTION', 'st_resize (
	...
	algorithm text , maxerr double precision )');
SELECT _postgis_package_object('FUNCTION', 'st_mapalgebraexpr (rast raster, band integer, pixeltype text,
        expression text, nodataval double precision )');
SELECT _postgis_package_object('FUNCTION', 'st_mapalgebraexpr (rast raster, pixeltype text, expression text,
        nodataval double precision )');
SELECT _postgis_package_object('FUNCTION', 'st_mapalgebrafct (rast raster, band integer,
...
SELECT _postgis_package_object('FUNCTION', 'st_mapalgebrafct (rast raster, pixeltype text,
        onerastuserfunc regprocedure, variadic args text[])');
SELECT _postgis_package_object('FUNCTION', 'st_mapalgebrafct (rast raster, pixeltype text,
        onerastuserfunc regprocedure)');
SELECT _postgis_package_object('FUNCTION', 'st_mapalgebrafct (rast raster, onerastuserfunc regprocedure,
        variadic args text[])');
SELECT _postgis_package_object('FUNCTION', 'st_mapalgebrafct (rast raster, onerastuserfunc regprocedure)');
SELECT _postgis_package_object('FUNCTION', 'st_mapalgebraexpr (
...
SELECT _postgis_package_object('FUNCTION', 'st_mapalgebraexpr (
	...
	nodata1expr text , nodata2expr text ,
	nodatanodataval double precision )');
SELECT _postgis_package_object('FUNCTION', 'st_mapalgebrafct (
	rast1 raster, band1 integer,
	rast2 raster, band2 integer,
...
SELECT _postgis_package_object('FUNCTION', 'st_mapalgebrafct (
	...
	pixeltype text , extenttype text ,
	VARIADIC userargs text[] )');
SELECT _postgis_package_object('FUNCTION', 'st_mapalgebrafctngb (
    rast raster,
    band integer,
    pixeltype text,
    ngbwidth integer,
...
SELECT _postgis_package_object('FUNCTION', 'st_max4ma (matrix float[][], nodatamode text, variadic args text[])');
SELECT _postgis_package_object('FUNCTION', 'st_min4ma (matrix float[][], nodatamode text, variadic args text[])');
SELECT _postgis_package_object('FUNCTION', 'st_sum4ma (matrix float[][], nodatamode text, variadic args text[])');
SELECT _postgis_package_object('FUNCTION', 'st_mean4ma (matrix float[][], nodatamode text, variadic args text[])');
SELECT _postgis_package_object('FUNCTION', 'st_range4ma (matrix float[][], nodatamode text, variadic args text[])');
SELECT _postgis_package_object('FUNCTION', 'st_distinct4ma (matrix float[][], nodatamode TEXT, VARIADIC args TEXT[])');
SELECT _postgis_package_object('FUNCTION', 'st_stddev4ma (matrix float[][], nodatamode TEXT, VARIADIC args TEXT[])');
SELECT _postgis_package_object('FUNCTION', '_st_mapalgebra (
	...
	VARIADIC userargs text[] )');
SELECT _postgis_package_object('FUNCTION', 'st_mapalgebra (
	rastbandargset rastbandarg[],
	callbackfunc regprocedure,
	pixeltype text ,
	extenttype text , customextent raster ,
...
SELECT _postgis_package_object('FUNCTION', 'st_mapalgebra (
	...
	nodata1expr text , nodata2expr text ,
	nodatanodataval double precision )');
SELECT _postgis_package_object('FUNCTION', '_st_convertarray4ma (value double precision[][])');
SELECT _postgis_package_object('FUNCTION', 'st_max4ma (value double precision[][][], pos integer[][], VARIADIC userargs text[] )');
SELECT _postgis_package_object('FUNCTION', 'st_min4ma (value double precision[][][], pos integer[][], VARIADIC userargs text[] )');
SELECT _postgis_package_object('FUNCTION', 'st_sum4ma (value double precision[][][], pos integer[][], VARIADIC userargs text[] )');
SELECT _postgis_package_object('FUNCTION', 'st_mean4ma (value double precision[][][], pos integer[][], VARIADIC userargs text[] )');
SELECT _postgis_package_object('FUNCTION', 'st_range4ma (value double precision[][][], pos integer[][], VARIADIC userargs text[] )');
SELECT _postgis_package_object('FUNCTION', 'st_distinct4ma (value double precision[][][], pos integer[][], VARIADIC userargs text[] )');
SELECT _postgis_package_object('FUNCTION', 'st_stddev4ma (value double precision[][][], pos integer[][], VARIADIC userargs text[] )');
SELECT _postgis_package_object('FUNCTION', 'st_invdistweight4ma (value double precision[][][], pos integer[][], VARIADIC userargs text[] )');
SELECT _postgis_package_object('FUNCTION', 'st_mindist4ma (value double precision[][][], pos integer[][], VARIADIC userargs text[] )');
SELECT _postgis_package_object('FUNCTION', '_st_slope4ma (value double precision[][][], pos integer[][], VARIADIC userargs text[] )');
SELECT _postgis_package_object('FUNCTION', 'st_slope (
	rast raster, nband integer,
	customextent raster,
...
SELECT _postgis_package_object('FUNCTION', 'st_slope (
	...
	pixeltype text , units text ,
	scale double precision ,	interpolate_nodata boolean )');
SELECT _postgis_package_object('FUNCTION', '_st_aspect4ma (value double precision[][][], pos integer[][], VARIADIC userargs text[] )');
SELECT _postgis_package_object('FUNCTION', 'st_aspect (
	rast raster, nband integer,
	customextent raster,
	pixeltype text , units text ,
...
SELECT _postgis_package_object('FUNCTION', 'st_aspect (
	...
	pixeltype text , units text ,
	interpolate_nodata boolean )');
SELECT _postgis_package_object('FUNCTION', '_st_hillshade4ma (value double precision[][][], pos integer[][], VARIADIC userargs text[] )');
SELECT _postgis_package_object('FUNCTION', 'st_hillshade (
	rast raster, nband integer,
	customextent raster,
	pixeltype text ,
...
SELECT _postgis_package_object('FUNCTION', 'st_hillshade (
	...
	max_bright double precision , scale double precision ,
	interpolate_nodata boolean )');
SELECT _postgis_package_object('FUNCTION', '_st_tpi4ma (value double precision[][][], pos integer[][], VARIADIC userargs text[] )');
SELECT _postgis_package_object('FUNCTION', 'st_tpi (
	rast raster, nband integer,
	customextent raster,
...
SELECT _postgis_package_object('FUNCTION', 'st_tpi (
	rast raster, nband integer ,
	pixeltype text , interpolate_nodata boolean )');
SELECT _postgis_package_object('FUNCTION', '_st_roughness4ma (value double precision[][][], pos integer[][], VARIADIC userargs text[] )');
SELECT _postgis_package_object('FUNCTION', 'st_roughness (
	rast raster, nband integer,
	customextent raster,
	pixeltype text , interpolate_nodata boolean )');
SELECT _postgis_package_object('FUNCTION', 'ST_roughness (
	rast raster, nband integer ,
	pixeltype text , interpolate_nodata boolean )');
SELECT _postgis_package_object('FUNCTION', '_st_tri4ma (value double precision[][][], pos integer[][], VARIADIC userargs text[] )');
SELECT _postgis_package_object('FUNCTION', 'st_tri (
	rast raster, nband integer,
	customextent raster,
	pixeltype text ,	interpolate_nodata boolean )');
SELECT _postgis_package_object('FUNCTION', 'st_tri (
	rast raster, nband integer ,
	pixeltype text , interpolate_nodata boolean )');
SELECT _postgis_package_object('FUNCTION', '_st_grayscale4ma (value double precision[][][], pos integer[][], VARIADIC userargs text[] )');
SELECT _postgis_package_object('FUNCTION', 'st_grayscale (
	rastbandargset rastbandarg[],
	extenttype text )');
SELECT _postgis_package_object('FUNCTION', 'st_grayscale (
	rast raster,
 	...
 	blueband integer ,
	extenttype text )');
SELECT _postgis_package_object('FUNCTION', 'st_isempty (rast raster)');
SELECT _postgis_package_object('FUNCTION', 'st_hasnoband (rast raster, nband int )');
SELECT _postgis_package_object('FUNCTION', 'st_bandnodatavalue (rast raster, band integer )');
SELECT _postgis_package_object('FUNCTION', 'st_bandisnodata (rast raster, band integer , forceChecking boolean )');
SELECT _postgis_package_object('FUNCTION', 'st_bandisnodata (rast raster, forceChecking boolean)');
SELECT _postgis_package_object('FUNCTION', 'st_bandpath (rast raster, band integer )');
SELECT _postgis_package_object('FUNCTION', 'ST_BandPixelType (rast raster, band integer )');
SELECT _postgis_package_object('FUNCTION', 'ST_BandMetaData (
	rast raster,
	band int[])');
SELECT _postgis_package_object('FUNCTION', 'ST_BandMetaData (
	rast raster,
	band int )');
SELECT _postgis_package_object('FUNCTION', 'ST_BandFileSize (rast raster, band integer )');
SELECT _postgis_package_object('FUNCTION', 'St_BandFileTimestamp (rast raster, band integer )');
SELECT _postgis_package_object('FUNCTION', 'st_value (rast raster, band integer, x integer, y integer, exclude_nodata_value boolean )');
SELECT _postgis_package_object('FUNCTION', 'st_value (rast raster, band integer, pt geometry, exclude_nodata_value boolean , resample text )');
SELECT _postgis_package_object('FUNCTION', 'st_value (rast raster, x integer, y integer, exclude_nodata_value boolean )');
SELECT _postgis_package_object('FUNCTION', 'st_value (rast raster, pt geometry, exclude_nodata_value boolean )');
SELECT _postgis_package_object('FUNCTION', 'st_setz (rast raster, geom geometry, resample text , band integer )');
SELECT _postgis_package_object('FUNCTION', 'st_setm (rast raster, geom geometry, resample text , band integer )');
SELECT _postgis_package_object('FUNCTION', 'st_pixelofvalue (
	rast raster,
	nband integer,
...
SELECT _postgis_package_object('FUNCTION', 'st_pixelofvalue (
	...
	search double precision,
	exclude_nodata_value boolean )');
SELECT _postgis_package_object('FUNCTION', 'st_georeference (rast raster, format text )');
SELECT _postgis_package_object('FUNCTION', 'st_setscale (rast raster, scale float8)');
SELECT _postgis_package_object('FUNCTION', 'st_setscale (rast raster, scalex float8, scaley float8)');
SELECT _postgis_package_object('FUNCTION', 'st_setskew (rast raster, skew float8)');
SELECT _postgis_package_object('FUNCTION', 'st_setskew (rast raster, skewx float8, skewy float8)');
SELECT _postgis_package_object('FUNCTION', 'st_setsrid (rast raster, srid integer)');
SELECT _postgis_package_object('FUNCTION', 'st_setupperleft (rast raster, upperleftx float8, upperlefty float8)');
SELECT _postgis_package_object('FUNCTION', 'st_setrotation (rast raster, rotation float8)');
SELECT _postgis_package_object('FUNCTION', 'st_setgeotransform (rast raster,
...
SELECT _postgis_package_object('FUNCTION', 'st_setgeoreference (rast raster, georef text, format text )');
SELECT _postgis_package_object('FUNCTION', 'st_setgeoreference (
	rast raster,
	upperleftx double precision, upperlefty double precision,
	scalex double precision, scaley double precision,
	skewx double precision, skewy double precision
)');
SELECT _postgis_package_object('FUNCTION', '_st_tile (
	rast raster,
	width integer, height integer,
	nband integer[] ,
	padwithnodata boolean , nodataval double precision )');
SELECT _postgis_package_object('FUNCTION', 'st_tile (
	rast raster, nband integer[],
	width integer, height integer,
	padwithnodata boolean , nodataval double precision )');
SELECT _postgis_package_object('FUNCTION', 'st_tile (
...
SELECT _postgis_package_object('FUNCTION', 'st_tile (
	...
	width integer, height integer,
	padwithnodata boolean , nodataval double precision )');
SELECT _postgis_package_object('FUNCTION', 'st_setbandnodatavalue (rast raster, band integer, nodatavalue float8, forceChecking boolean )');
SELECT _postgis_package_object('FUNCTION', 'st_setbandnodatavalue (rast raster, nodatavalue float8)');
SELECT _postgis_package_object('FUNCTION', 'st_setbandisnodata (rast raster, band integer )');
SELECT _postgis_package_object('FUNCTION', 'st_setbandpath (rast raster, band integer, outdbpath text, outdbindex integer, force boolean )');
SELECT _postgis_package_object('FUNCTION', 'st_setbandindex (rast raster, band integer, outdbindex integer, force boolean )');
SELECT _postgis_package_object('FUNCTION', '_ST_SetValues (
	rast raster, nband integer,
	x integer, y integer,
	...
	keepnodata boolean )');
SELECT _postgis_package_object('FUNCTION', 'ST_SetValues (
	rast raster, nband integer,
	x integer, y integer,
	newvalueset double precision[][],
	noset boolean[][] ,
...
SELECT _postgis_package_object('FUNCTION', 'ST_SetValues (
	...
	geomvalset geomval[],
	keepnodata boolean )');
SELECT _postgis_package_object('FUNCTION', 'ST_SetValue (rast raster, band integer, x integer, y integer, newvalue float8)');
SELECT _postgis_package_object('FUNCTION', 'ST_SetValue (rast raster, x integer, y integer, newvalue float8)');
SELECT _postgis_package_object('FUNCTION', 'ST_SetValue (
	rast raster, nband integer,
...
SELECT _postgis_package_object('FUNCTION', 'st_dumpaspolygons (rast raster, band integer , exclude_nodata_value boolean )');
SELECT _postgis_package_object('FUNCTION', 'st_dumpvalues (
	rast raster, nband integer[] , exclude_nodata_value boolean )');
SELECT _postgis_package_object('FUNCTION', 'st_dumpvalues (rast raster, nband integer, exclude_nodata_value boolean )');
SELECT _postgis_package_object('FUNCTION', 'st_polygon (rast raster, band integer )');
SELECT _postgis_package_object('FUNCTION', '_st_pixelaspolygons (
	rast raster,
	band integer ,
	columnx integer ,
	rowy integer ,
	exclude_nodata_value boolean )');
SELECT _postgis_package_object('FUNCTION', 'st_pixelaspolygons (
	rast raster,
	band integer ,
	exclude_nodata_value boolean )');
SELECT _postgis_package_object('FUNCTION', 'st_pixelaspolygon (rast raster, x integer, y integer)');
SELECT _postgis_package_object('FUNCTION', 'st_pixelaspoints (
	rast raster,
	band integer ,
	exclude_nodata_value boolean )');
SELECT _postgis_package_object('FUNCTION', 'st_pixelaspoint (rast raster, x integer, y integer)');
SELECT _postgis_package_object('FUNCTION', '_st_pixelascentroids (
	rast raster,
	band integer ,
	columnx integer ,
	rowy integer ,
	exclude_nodata_value boolean )');
SELECT _postgis_package_object('FUNCTION', 'st_pixelascentroids (
	rast raster,
	band integer ,
	exclude_nodata_value boolean )');
SELECT _postgis_package_object('FUNCTION', 'st_pixelascentroid (rast raster, x integer, y integer)');
SELECT _postgis_package_object('FUNCTION', '_st_worldtorastercoord (
	rast raster,
	longitude double precision , latitude double precision ,
	OUT columnx integer,
	OUT rowy integer)');
SELECT _postgis_package_object('FUNCTION', 'st_worldtorastercoord (
	rast raster,
	longitude double precision, latitude double precision,
	OUT columnx integer,
	OUT rowy integer)');
SELECT _postgis_package_object('FUNCTION', 'st_worldtorastercoord (
	...
	OUT columnx integer,
	OUT rowy integer)');
SELECT _postgis_package_object('FUNCTION', 'st_worldtorastercoordx (rast raster, xw float8, yw float8)');
SELECT _postgis_package_object('FUNCTION', 'st_worldtorastercoordx (rast raster, xw float8)');
SELECT _postgis_package_object('FUNCTION', 'st_worldtorastercoordx (rast raster, pt geometry)');
SELECT _postgis_package_object('FUNCTION', 'st_worldtorastercoordy (rast raster, xw float8, yw float8)');
SELECT _postgis_package_object('FUNCTION', 'st_worldtorastercoordy (rast raster, yw float8)');
SELECT _postgis_package_object('FUNCTION', 'st_worldtorastercoordy (rast raster, pt geometry)');
SELECT _postgis_package_object('FUNCTION', '_st_rastertoworldcoord (
	rast raster,
	columnx integer , rowy integer ,
	OUT longitude double precision,
	OUT latitude double precision
	)');
SELECT _postgis_package_object('FUNCTION', 'st_rastertoworldcoord (
	rast raster,
	columnx integer, rowy integer,
	OUT	longitude double precision,
	OUT latitude double precision
	)');
SELECT _postgis_package_object('FUNCTION', 'st_rastertoworldcoordx (rast raster, xr int, yr int)');
SELECT _postgis_package_object('FUNCTION', 'st_rastertoworldcoordx (rast raster, xr int)');
SELECT _postgis_package_object('FUNCTION', 'st_rastertoworldcoordy (rast raster, xr int, yr int)');
SELECT _postgis_package_object('FUNCTION', 'st_rastertoworldcoordy (rast raster, yr int)');
SELECT _postgis_package_object('FUNCTION', 'st_minpossiblevalue (pixeltype text)');
SELECT _postgis_package_object('FUNCTION', 'st_rastfromwkb (bytea)');
SELECT _postgis_package_object('FUNCTION', 'st_rastfromhexwkb (text)');
SELECT _postgis_package_object('FUNCTION', 'st_aswkb (raster, outasin boolean )');
SELECT _postgis_package_object('FUNCTION', 'st_asbinary (raster, outasin boolean )');
SELECT _postgis_package_object('FUNCTION', 'st_ashexwkb (raster, outasin boolean )');
SELECT _postgis_package_object('FUNCTION', 'bytea (raster)');
SELECT _postgis_package_object('FUNCTION', 'raster_hash (raster)');
SELECT _postgis_package_object('FUNCTION', 'raster_eq (raster, raster)');
SELECT _postgis_package_object('FUNCTION', 'raster_overleft (raster, raster)');
SELECT _postgis_package_object('FUNCTION', 'raster_overright (raster, raster)');
SELECT _postgis_package_object('FUNCTION', 'raster_left (raster, raster)');
SELECT _postgis_package_object('FUNCTION', 'raster_right (raster, raster)');
SELECT _postgis_package_object('FUNCTION', 'raster_overabove (raster, raster)');
SELECT _postgis_package_object('FUNCTION', 'raster_overbelow (raster, raster)');
SELECT _postgis_package_object('FUNCTION', 'raster_above (raster, raster)');
SELECT _postgis_package_object('FUNCTION', 'raster_below (raster, raster)');
SELECT _postgis_package_object('FUNCTION', 'raster_same (raster, raster)');
SELECT _postgis_package_object('FUNCTION', 'raster_contained (raster, raster)');
SELECT _postgis_package_object('FUNCTION', 'raster_contain (raster, raster)');
SELECT _postgis_package_object('FUNCTION', 'raster_overlap (raster, raster)');
SELECT _postgis_package_object('FUNCTION', 'raster_geometry_contain (raster, geometry)');
SELECT _postgis_package_object('FUNCTION', 'raster_contained_by_geometry (raster, geometry)');
SELECT _postgis_package_object('FUNCTION', 'raster_geometry_overlap (raster, geometry)');
SELECT _postgis_package_object('FUNCTION', 'geometry_raster_contain (geometry, raster)');
SELECT _postgis_package_object('FUNCTION', 'geometry_contained_by_raster (geometry, raster)');
SELECT _postgis_package_object('FUNCTION', 'geometry_raster_overlap (geometry, raster)');
SELECT _postgis_package_object('FUNCTION', 'st_samealignment (rast1 raster, rast2 raster)');
SELECT _postgis_package_object('FUNCTION', 'st_samealignment (
...
SELECT _postgis_package_object('FUNCTION', '_st_samealignment_transfn (agg agg_samealignment, rast raster)');
SELECT _postgis_package_object('FUNCTION', '_ST_samealignment_finalfn (agg agg_samealignment)');
SELECT _postgis_package_object('FUNCTION', 'st_notsamealignmentreason (rast1 raster, rast2 raster)');
SELECT _postgis_package_object('FUNCTION', 'st_iscoveragetile (rast raster, coverage raster, tilewidth integer, tileheight integer)');
SELECT _postgis_package_object('FUNCTION', '_st_intersects (rast1 raster, nband1 integer, rast2 raster, nband2 integer)');
SELECT _postgis_package_object('FUNCTION', 'st_intersects (rast1 raster, nband1 integer, rast2 raster, nband2 integer)');
SELECT _postgis_package_object('FUNCTION', 'st_intersects (rast1 raster, rast2 raster)');
SELECT _postgis_package_object('FUNCTION', '_st_intersects (geom geometry, rast raster, nband integer )');
SELECT _postgis_package_object('FUNCTION', 'st_intersects (geom geometry, rast raster, nband integer )');
SELECT _postgis_package_object('FUNCTION', 'st_intersects (rast raster, geom geometry, nband integer )');
SELECT _postgis_package_object('FUNCTION', 'st_intersects (rast raster, nband integer, geom geometry)');
SELECT _postgis_package_object('FUNCTION', '_st_overlaps (rast1 raster, nband1 integer, rast2 raster, nband2 integer)');
SELECT _postgis_package_object('FUNCTION', 'st_overlaps (rast1 raster, nband1 integer, rast2 raster, nband2 integer)');
SELECT _postgis_package_object('FUNCTION', 'st_overlaps (rast1 raster, rast2 raster)');
SELECT _postgis_package_object('FUNCTION', '_st_touches (rast1 raster, nband1 integer, rast2 raster, nband2 integer)');
SELECT _postgis_package_object('FUNCTION', 'st_touches (rast1 raster, nband1 integer, rast2 raster, nband2 integer)');
SELECT _postgis_package_object('FUNCTION', 'st_touches (rast1 raster, rast2 raster)');
SELECT _postgis_package_object('FUNCTION', '_st_contains (rast1 raster, nband1 integer, rast2 raster, nband2 integer)');
SELECT _postgis_package_object('FUNCTION', 'st_contains (rast1 raster, nband1 integer, rast2 raster, nband2 integer)');
SELECT _postgis_package_object('FUNCTION', 'st_contains (rast1 raster, rast2 raster)');
SELECT _postgis_package_object('FUNCTION', '_st_containsproperly (rast1 raster, nband1 integer, rast2 raster, nband2 integer)');
SELECT _postgis_package_object('FUNCTION', 'st_containsproperly (rast1 raster, nband1 integer, rast2 raster, nband2 integer)');
SELECT _postgis_package_object('FUNCTION', 'st_containsproperly (rast1 raster, rast2 raster)');
SELECT _postgis_package_object('FUNCTION', '_st_covers (rast1 raster, nband1 integer, rast2 raster, nband2 integer)');
SELECT _postgis_package_object('FUNCTION', 'st_covers (rast1 raster, nband1 integer, rast2 raster, nband2 integer)');
SELECT _postgis_package_object('FUNCTION', 'st_covers (rast1 raster, rast2 raster)');
SELECT _postgis_package_object('FUNCTION', '_st_coveredby (rast1 raster, nband1 integer, rast2 raster, nband2 integer)');
SELECT _postgis_package_object('FUNCTION', 'st_coveredby (rast1 raster, nband1 integer, rast2 raster, nband2 integer)');
SELECT _postgis_package_object('FUNCTION', 'st_coveredby (rast1 raster, rast2 raster)');
SELECT _postgis_package_object('FUNCTION', '_st_within (rast1 raster, nband1 integer, rast2 raster, nband2 integer)');
SELECT _postgis_package_object('FUNCTION', 'st_within (rast1 raster, nband1 integer, rast2 raster, nband2 integer)');
SELECT _postgis_package_object('FUNCTION', 'st_within (rast1 raster, rast2 raster)');
SELECT _postgis_package_object('FUNCTION', '_ST_DWithin (rast1 raster, nband1 integer, rast2 raster, nband2 integer, distance double precision)');
SELECT _postgis_package_object('FUNCTION', 'ST_DWithin (rast1 raster, nband1 integer, rast2 raster, nband2 integer, distance double precision)');
SELECT _postgis_package_object('FUNCTION', 'ST_DWithin (rast1 raster, rast2 raster, distance double precision)');
SELECT _postgis_package_object('FUNCTION', '_ST_DFullyWithin (rast1 raster, nband1 integer, rast2 raster, nband2 integer, distance double precision)');
SELECT _postgis_package_object('FUNCTION', 'ST_DFullyWithin (rast1 raster, nband1 integer, rast2 raster, nband2 integer, distance double precision)');
SELECT _postgis_package_object('FUNCTION', 'ST_DFullyWithin (rast1 raster, rast2 raster, distance double precision)');
SELECT _postgis_package_object('FUNCTION', 'st_disjoint (rast1 raster, nband1 integer, rast2 raster, nband2 integer)');
SELECT _postgis_package_object('FUNCTION', 'ST_Disjoint (rast1 raster, rast2 raster)');
SELECT _postgis_package_object('FUNCTION', 'ST_Intersection (geomin geometry, rast raster, band integer )');
SELECT _postgis_package_object('FUNCTION', 'st_intersection (rast raster, band integer, geomin geometry)');
SELECT _postgis_package_object('FUNCTION', 'ST_Intersection (rast raster, geomin geometry)');
SELECT _postgis_package_object('FUNCTION', 'ST_Intersection (
...
SELECT _postgis_package_object('FUNCTION', '_st_union_finalfn (internal)');
SELECT _postgis_package_object('FUNCTION', '_st_union_transfn (internal, raster, unionarg[])');
SELECT _postgis_package_object('FUNCTION', '_st_union_transfn (internal, raster, integer, text)');
SELECT _postgis_package_object('FUNCTION', '_st_union_transfn (internal, raster, integer)');
SELECT _postgis_package_object('FUNCTION', '_st_union_transfn (internal, raster)');
SELECT _postgis_package_object('FUNCTION', '_st_union_transfn (internal, raster, text)');
SELECT _postgis_package_object('FUNCTION', '_st_clip (
	rast raster, nband integer[],
	geom geometry,
	nodataval double precision[] , crop boolean )');
SELECT _postgis_package_object('FUNCTION', 'st_clip (
	rast raster, nband integer[],
	geom geometry,
	nodataval double precision[] , crop boolean )');
SELECT _postgis_package_object('FUNCTION', 'st_clip (
...
SELECT _postgis_package_object('FUNCTION', 'st_nearestvalue (
...
SELECT _postgis_package_object('FUNCTION', 'st_nearestvalue (
	...
	columnx integer, rowy integer,
	exclude_nodata_value boolean )');
SELECT _postgis_package_object('FUNCTION', '_st_neighborhood (
	rast raster, band integer,
	columnx integer, rowy integer,
	distancex integer, distancey integer,
	exclude_nodata_value boolean )');
SELECT _postgis_package_object('FUNCTION', 'st_neighborhood (
	rast raster, band integer,
	columnx integer, rowy integer,
	distancex integer, distancey integer,
	exclude_nodata_value boolean )');
...
SELECT _postgis_package_object('FUNCTION', 'st_neighborhood (
	...
	distancex integer, distancey integer,
	exclude_nodata_value boolean )');
SELECT _postgis_package_object('FUNCTION', '_add_raster_constraint (cn name, sql text)');
SELECT _postgis_package_object('FUNCTION', '_drop_raster_constraint (rastschema name, rasttable name, cn name)');
SELECT _postgis_package_object('FUNCTION', '_raster_constraint_info_srid (rastschema name, rasttable name, rastcolumn name)');
SELECT _postgis_package_object('FUNCTION', '_add_raster_constraint_srid (rastschema name, rasttable name, rastcolumn name)');
SELECT _postgis_package_object('FUNCTION', '_drop_raster_constraint_srid (rastschema name, rasttable name, rastcolumn name)');
SELECT _postgis_package_object('FUNCTION', '_raster_constraint_info_scale (rastschema name, rasttable name, rastcolumn name, axis char)');
SELECT _postgis_package_object('FUNCTION', '_add_raster_constraint_scale (rastschema name, rasttable name, rastcolumn name, axis char)');
SELECT _postgis_package_object('FUNCTION', '_drop_raster_constraint_scale (rastschema name, rasttable name, rastcolumn name, axis char)');
SELECT _postgis_package_object('FUNCTION', '_raster_constraint_info_blocksize (rastschema name, rasttable name, rastcolumn name, axis text)');
SELECT _postgis_package_object('FUNCTION', '_add_raster_constraint_blocksize (rastschema name, rasttable name, rastcolumn name, axis text)');
SELECT _postgis_package_object('FUNCTION', '_drop_raster_constraint_blocksize (rastschema name, rasttable name, rastcolumn name, axis text)');
SELECT _postgis_package_object('FUNCTION', '_raster_constraint_info_extent (rastschema name, rasttable name, rastcolumn name)');
SELECT _postgis_package_object('FUNCTION', '_add_raster_constraint_extent (rastschema name, rasttable name, rastcolumn name)');
SELECT _postgis_package_object('FUNCTION', '_drop_raster_constraint_extent (rastschema name, rasttable name, rastcolumn name)');
SELECT _postgis_package_object('FUNCTION', '_raster_constraint_info_alignment (rastschema name, rasttable name, rastcolumn name)');
SELECT _postgis_package_object('FUNCTION', '_add_raster_constraint_alignment (rastschema name, rasttable name, rastcolumn name)');
SELECT _postgis_package_object('FUNCTION', '_drop_raster_constraint_alignment (rastschema name, rasttable name, rastcolumn name)');
SELECT _postgis_package_object('FUNCTION', '_raster_constraint_info_spatially_unique (rastschema name, rasttable name, rastcolumn name)');
SELECT _postgis_package_object('FUNCTION', '_add_raster_constraint_spatially_unique (rastschema name, rasttable name, rastcolumn name)');
SELECT _postgis_package_object('FUNCTION', '_drop_raster_constraint_spatially_unique (rastschema name, rasttable name, rastcolumn name)');
SELECT _postgis_package_object('FUNCTION', '_raster_constraint_info_coverage_tile (rastschema name, rasttable name, rastcolumn name)');
SELECT _postgis_package_object('FUNCTION', '_add_raster_constraint_coverage_tile (rastschema name, rasttable name, rastcolumn name)');
SELECT _postgis_package_object('FUNCTION', '_drop_raster_constraint_coverage_tile (rastschema name, rasttable name, rastcolumn name)');
SELECT _postgis_package_object('FUNCTION', '_raster_constraint_info_regular_blocking (rastschema name, rasttable name, rastcolumn name)');
SELECT _postgis_package_object('FUNCTION', '_drop_raster_constraint_regular_blocking (rastschema name, rasttable name, rastcolumn name)');
SELECT _postgis_package_object('FUNCTION', '_raster_constraint_info_num_bands (rastschema name, rasttable name, rastcolumn name)');
SELECT _postgis_package_object('FUNCTION', '_add_raster_constraint_num_bands (rastschema name, rasttable name, rastcolumn name)');
SELECT _postgis_package_object('FUNCTION', '_drop_raster_constraint_num_bands (rastschema name, rasttable name, rastcolumn name)');
SELECT _postgis_package_object('FUNCTION', '_raster_constraint_info_pixel_types (rastschema name, rasttable name, rastcolumn name)');
SELECT _postgis_package_object('FUNCTION', '_raster_constraint_pixel_types (rast raster)');
SELECT _postgis_package_object('FUNCTION', '_add_raster_constraint_pixel_types (rastschema name, rasttable name, rastcolumn name)');
SELECT _postgis_package_object('FUNCTION', '_drop_raster_constraint_pixel_types (rastschema name, rasttable name, rastcolumn name)');
SELECT _postgis_package_object('FUNCTION', '_raster_constraint_info_nodata_values (rastschema name, rasttable name, rastcolumn name)');
SELECT _postgis_package_object('FUNCTION', '_raster_constraint_nodata_values (rast raster)');
SELECT _postgis_package_object('FUNCTION', '_add_raster_constraint_nodata_values (rastschema name, rasttable name, rastcolumn name)');
SELECT _postgis_package_object('FUNCTION', '_drop_raster_constraint_nodata_values (rastschema name, rasttable name, rastcolumn name)');
SELECT _postgis_package_object('FUNCTION', '_raster_constraint_info_out_db (rastschema name, rasttable name, rastcolumn name)');
SELECT _postgis_package_object('FUNCTION', '_raster_constraint_out_db (rast raster)');
SELECT _postgis_package_object('FUNCTION', '_add_raster_constraint_out_db (rastschema name, rasttable name, rastcolumn name)');
SELECT _postgis_package_object('FUNCTION', '_drop_raster_constraint_out_db (rastschema name, rasttable name, rastcolumn name)');
SELECT _postgis_package_object('FUNCTION', '_raster_constraint_info_index (rastschema name, rasttable name, rastcolumn name)');
SELECT _postgis_package_object('FUNCTION', 'AddRasterConstraints  (
	rastschema name,
	rasttable name,
	rastcolumn name,
...
SELECT _postgis_package_object('FUNCTION', 'AddRasterConstraints  (
	...
	out_db boolean ,
	extent boolean )');
SELECT _postgis_package_object('FUNCTION', 'DropRasterConstraints  (
	rastschema name,
	rasttable name,
	rastcolumn name,
	VARIADIC constraints text[]
...
SELECT _postgis_package_object('FUNCTION', 'DropRasterConstraints  (
	...
	out_db boolean ,
	extent boolean )');
SELECT _postgis_package_object('FUNCTION', '_overview_constraint (ov raster, factor integer, refschema name, reftable name, refcolumn name)');
SELECT _postgis_package_object('FUNCTION', '_overview_constraint_info (
...
SELECT _postgis_package_object('FUNCTION', '_add_overview_constraint (
	ovschema name, ovtable name, ovcolumn name,
	refschema name, reftable name, refcolumn name,
	factor integer
)');
SELECT _postgis_package_object('FUNCTION', '_drop_overview_constraint (ovschema name, ovtable name, ovcolumn name)');
SELECT _postgis_package_object('FUNCTION', 'AddOverviewConstraints  (
	ovschema name, ovtable name, ovcolumn name,
	refschema name, reftable name, refcolumn name,
	ovfactor int
)');
...
SELECT _postgis_package_object('FUNCTION', 'DropOverviewConstraints  (
	ovschema name,
	ovtable name,
	ovcolumn name
)');
...
SELECT _postgis_package_object('FUNCTION', '_UpdateRasterSRID (
	schema_name name, table_name name, column_name name,
	new_srid integer
)');
SELECT _postgis_package_object('FUNCTION', 'UpdateRasterSRID (
	schema_name name, table_name name, column_name name,
	new_srid integer
)');
SELECT _postgis_package_object('FUNCTION', 'UpdateRasterSRID (
...
SELECT _postgis_package_object('FUNCTION', 'ST_Retile (tab regclass, col name, ext geometry, sfx float8, sfy float8, tw int, th int, algo text )');
SELECT _postgis_package_object('FUNCTION', 'ST_CreateOverview (tab regclass, col name, factor int, algo text )');
SELECT _postgis_package_object('FUNCTION', 'st_makeemptycoverage (tilewidth int, tileheight int, width int, height int, upperleftx float8, upperlefty float8, scalex float8, scaley float8, skewx float8, skewy float8, srid integer )');
SELECT _postgis_package_object('FUNCTION', 'postgis_noop (raster)');
-- Register all types.
SELECT _postgis_package_object('TYPE', 'raster');
SELECT _postgis_package_object('TYPE', 'rastbandarg');
SELECT _postgis_package_object('TYPE', 'geomval');
SELECT _postgis_package_object('TYPE', 'addbandarg');
SELECT _postgis_package_object('TYPE', 'summarystats');
SELECT _postgis_package_object('TYPE', 'agg_count');
SELECT _postgis_package_object('TYPE', 'reclassarg');
SELECT _postgis_package_object('TYPE', 'agg_samealignment');
SELECT _postgis_package_object('TYPE', 'unionarg');
...
BEGIN
    ...
    old_scripts := postgis_raster_scripts_installed();
    new_scripts := '3.3.4';
...
CREATE OR REPLACE FUNCTION st_samealignment(
	...
	RETURNS boolean
	AS $$ SELECT @extschema@.st_samealignment(@extschema@.st_makeemptyraster(1, 1, $1, $2, $3, $4, $5, $6), @extschema@.st_makeemptyraster(1, 1, $7, $8, $9, $10, $11, $12)) $$
	LANGUAGE 'sql' IMMUTABLE STRICT PARALLEL SAFE;
	...
	BEGIN
		...
		ELSE
			IF agg.refraster IS NULL THEN
				m := @extschema@.ST_Metadata(rast);
				agg.refraster := @extschema@.ST_MakeEmptyRaster(1, 1, m.upperleftx, m.upperlefty, m.scalex, m.scaley, m.skewx, m.skewy, m.srid);
				agg.aligned := TRUE;
			ELSIF agg.aligned IS TRUE THEN
				agg.aligned := @extschema@.ST_SameAlignment(agg.refraster, rast);
			END IF;
		END IF;
		RETURN agg;
	END;
...
CREATE OR REPLACE FUNCTION _add_raster_constraint_coverage_tile(rastschema name, rasttable name, rastcolumn name)
	...
	BEGIN
      ...
      -- TODO: Why not return false here ?
		...
		sql := 'ALTER TABLE ' || fqtn ||
			' ADD CONSTRAINT ' || quote_ident(cn) ||
			' CHECK (@extschema@.st_iscoveragetile(' || quote_ident($3) || ', ''' || _covrast || '''::raster, ' || _tilewidth || ', ' || _tileheight || '))';
		RETURN  @extschema@._add_raster_constraint(cn, sql);
	END;
	$$ LANGUAGE 'plpgsql' VOLATILE STRICT
	COST 100;
...
CREATE OR REPLACE FUNCTION _raster_constraint_pixel_types(rast raster)
	RETURNS text[] AS
	$$ SELECT pg_catalog.array_agg(pixeltype)::text[] FROM  @extschema@.ST_BandMetaData($1, ARRAY[]::int[]); $$
	LANGUAGE 'sql' STABLE STRICT;
...
CREATE OR REPLACE FUNCTION _raster_constraint_nodata_values(rast raster)
	RETURNS numeric[] AS
	$$ SELECT pg_catalog.array_agg(pg_catalog.round(nodatavalue::numeric, 10))::numeric[] FROM @extschema@.ST_BandMetaData($1, ARRAY[]::int[]); $$
	LANGUAGE 'sql' IMMUTABLE STRICT PARALLEL SAFE;
...
CREATE OR REPLACE FUNCTION _raster_constraint_out_db(rast raster)
	RETURNS boolean[] AS
	$$ SELECT pg_catalog.array_agg(isoutdb)::boolean[] FROM @extschema@.ST_BandMetaData($1, ARRAY[]::int[]); $$
	LANGUAGE 'sql' IMMUTABLE STRICT PARALLEL SAFE;
...
COMMENT ON FUNCTION ST_SummaryStatsAgg(setof raster , integer , boolean , double precision ) IS 'args: rast, nband, exclude_nodata_value, sample_percent - Aggregate. Returns summarystats consisting of count, sum, mean, stddev, min, max for a given raster band of a set of raster. Band 1 is assumed is no band is specified.';
			
COMMENT ON FUNCTION ST_SummaryStatsAgg(setof raster , boolean , double precision ) IS 'args: rast, exclude_nodata_value, sample_percent - Aggregate. Returns summarystats consisting of count, sum, mean, stddev, min, max for a given raster band of a set of raster. Band 1 is assumed is no band is specified.';
			
COMMENT ON FUNCTION ST_SummaryStatsAgg(setof raster , integer , boolean ) IS 'args: rast, nband, exclude_nodata_value - Aggregate. Returns summarystats consisting of count, sum, mean, stddev, min, max for a given raster band of a set of raster. Band 1 is assumed is no band is specified.';
			
COMMENT ON FUNCTION ST_ValueCount(setof raster , integer , boolean , double precision[] , double precision ) IS 'args: rast, nband=1, exclude_nodata_value=true, searchvalues=NULL, roundto=0, OUT value, OUT count - Returns a set of records containing a pixel band value and count of the number of pixels in a given band of a raster (or a raster coverage) that have a given set of values. If no band is specified defaults to band 1. By default nodata value pixels are not counted. and all other values in the pixel are output and pixel band values are rounded to the nearest integer.';
			
COMMENT ON FUNCTION ST_ValueCount(setof raster , integer , double precision[] , double precision ) IS 'args: rast, nband, searchvalues, roundto=0, OUT value, OUT count - Returns a set of records containing a pixel band value and count of the number of pixels in a given band of a raster (or a raster coverage) that have a given set of values. If no band is specified defaults to band 1. By default nodata value pixels are not counted. and all other values in the pixel are output and pixel band values are rounded to the nearest integer.';
			
COMMENT ON FUNCTION ST_ValueCount(setof raster , double precision[] , double precision ) IS 'args: rast, searchvalues, roundto=0, OUT value, OUT count - Returns a set of records containing a pixel band value and count of the number of pixels in a given band of a raster (or a raster coverage) that have a given set of values. If no band is specified defaults to band 1. By default nodata value pixels are not counted. and all other values in the pixel are output and pixel band values are rounded to the nearest integer.';
			
COMMENT ON FUNCTION ST_ValueCount(setof raster , double precision , double precision ) IS 'args: rast, searchvalue, roundto=0 - Returns a set of records containing a pixel band value and count of the number of pixels in a given band of a raster (or a raster coverage) that have a given set of values. If no band is specified defaults to band 1. By default nodata value pixels are not counted. and all other values in the pixel are output and pixel band values are rounded to the nearest integer.';
...
COMMENT ON FUNCTION ST_ValueCount(setof raster , integer , double precision , double precision ) IS 'args: rast, nband, searchvalue, roundto=0 - Returns a set of records containing a pixel band value and count of the number of pixels in a given band of a raster (or a raster coverage) that have a given set of values. If no band is specified defaults to band 1. By default nodata value pixels are not counted. and all other values in the pixel are output and pixel band values are rounded to the nearest integer.';
